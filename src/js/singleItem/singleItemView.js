export function render(object, isFaved) {
  // будет рендерить необходимую разметку
...
export function toggleFavouriteButton(isFaved) {
  // будет находить кнопку соответственно добавлять к ней активный класс, либо убирать его, менять текстовку
  const btn = document.querySelector("#addToFavouriteBtn");

  // Проверка true или false (элемент в избранном или нет);
  if (isFaved) {
